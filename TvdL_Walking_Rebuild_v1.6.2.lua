...
function Build(worklist, len, maxiters, sphere, lws, bf)
   for i=1,#worklist do
	if worklist[i] == nil then break end -- DEBUG
      local s1=worklist[i]
...
function AskWalking()
   ...
   if dialog.Show(ask) > 0 then
	  ...
	  if ask.inward.value then worklist=InwardTable(worklist) end
	  if ask.outward.value then worklist=OutwardTable(worklist) end
	  if ask.sliceward.value then worklist=MixInwardTable(worklist) end
