...
IterationMultiplier = 1.2 -- 2.2
--[[ Multiply wiggle iterations by 
[most used values of wiggle iterations are 4(perturb), 8(lower than maxCI), and 10(stabilize)]
 Recommended value between 0.8 and 2.5.
]]--

normal = true    -- false   -- set false to use .GetEnergyScore()
...
function score()
local s=0
	if normal==false then s=current.GetEnergyScore() else
s=current.GetScore() end
	return s
end
...
p(" Presets: ")
p(" Early Game: ")
p(" 1 = drw small rebuilds, shortest first - fast healing")  
p(" 2 = drw large-med rebuilds") 
p(" 3 = rr large-med rebuilds") 
p(" Mid Game: ")
p(" 1 = drw med rebuilds") 
p(" 2 = rr big-med rebuilds") 
p(" 3 = rr med-small rebuilds") 
p(" End Game: ")
p(" 1 = drw small rebuilds") 
p(" 2 = rr small rebuilds") 
p(" 3 = drw acid tweak") 
p(" 4 = rr acid tweak") 
p("")
p("")
...
if dehr == true then
p("   All three checked; trying both.")
end
if Hhrr == true then
p("   Nothing checked, pull, then wiggle only.")
end
if Uhr == true then
p("   Both checked, shake/mutate after pull. ")
end
...
save.Quicksave(8)
if runs>50 then
p(" Let's get things moving, shall we.")
p("",runs,"runs to go.")
end
...
if comdecom == true then
	if Bloat == true then
p(" Expand ",bandruns,"of",runs)
else
p(" Contract ",bandruns,"of",runs)
end
else
p(" Spacebands ",bandruns,"of",runs)
end
...
p(" Bander gain:",cut(BndrBestscore-BndrStartscore)," Last gain: Run",ZZ)
end
...
InBander = false
CheckBest()
p(" Gained:",cut(BndrBestscore-BndrStartscore))
CheckAla()
end
------------------------------------------------------------------------
checkLock()
...
p(" Best score will be saved in quickslot 1 ")
p(" Starting structure in quickslot 2 ")
if AllAla == true then
p("  No sidechains: no shaking, still mutating if checked.")
end
...
if RR == false then
if numruns<1 then
p(" "..numruns.." cycle of deep rebuild, coming up.")
else
p(" "..numruns.." cycles of deep rebuild, coming up.")
end
else
if numruns2<1 then
p(" "..numruns2.." cycle of rainbow rebuild, coming up.")
else
p(" "..numruns2.." cycles of rainbow rebuild, coming up.")
end
end
...
p("")
rcyc=rcyc+1
    p(" Cycle",rcyc,"of",numruns2-minusR)
	p(" Rebuild length: "..rbLng)
	p(" Range: "..startseg.."-"..endseg)
	p(" Starting score this cycle: "..cut(score()))
printRR=false
	end
rblds = rblds+1 
p(" Rebuild:",rblds,"Segments:",seg.."-"..seg+rbLng-1," "..cut(score()).." ",os.date("%X"))
xseg = seg
oldscore = score()
if AllLoop then
	for e=1, nSegs do
...
save.Quicksave(99)
save.Quicksave(8)
p(" Gained "..cut(score()-oldscore).." pts. this rebuild.")
else
end
end
...
save.Quickload(99)
  ...
  if (score()-cRscore)>1 then
		p(" Startscore: "..cut(startscore).."  Score: "..cut(score()))
		p(" Gained: "..cut((score()-cRscore)).." pts. this cycle.")
	noGain = false
	badcycles = 0
...
badcycles = badcycles+1
	if rcyc>0 then
		p(" Score: "..cut(score()), "  No gain this cycle.")
printRR=true
	else
numruns2 = numruns2 +1
minusR=minusR+1
...
if WorstFirst == true then
p(" Looking for worst "..SubscorePart.." score.")
else
p(" Looking for best "..SubscorePart.." score.")
end
	rblds = 0
	j=0
Jround=0
...
save.LoadSecondaryStructure()
		selection.DeselectAll()
	rblds = rblds+1
p(" Rebuild:",rblds, "Segments:",rbldStrt.."-"..rbldEnd," "..cut(score()).." ",os.date("%X"))
			selection.SelectRange(rbldStrt,rbldEnd)
if AllLoop then
	for e=1, nSegs do
if selection.IsSelected(e) then structure.SetSecondaryStructure(e, "L") end
...
--p("",rbldseg)
if gsort then 
p(" Target's score:",cut(TempSegSc)) end 
			structure.RebuildSelected(1)
			recentbest.Save()			
save.Quicksave(97)
...
p(" Gained "..cut(score()-qscore).." pts. this rebuild.")
	end
...
if (rbldcomp == true) and (rblds>=1) then Bander() end
	...
	p(" ",cntGary-garyCnt,"more good rebuilds before sort.") end end
	end
 if gsort then
 ...
 --if ReSort == true then SortItWell(SubscorePart) end
  ...
  if (score()-cscore)>0.1 then
		p(" Startscore: "..cut(startscore).."  Score: "..cut(score()))
		p(" Gained: "..cut((score()-cscore)).." pts. this cycle.")
	noGain = false
	goodcycles = goodcycles+1
	badcycles = 0
	else
